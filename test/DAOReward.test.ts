...
describe("DAO Reward System Tests", function () {
  ...
  async function deployRewardSystemFixture() {
    ...
    await GovernanceToken.write.mint([GovernorsRewardPay.address, parseEther("1000")]);
  ...
  describe("Reward Calculation", function () {
    it("Should calculate rewards correctly based on successful votes", async function () {
      ...
      // Set up voter1 to have 3 successful votes
      ...
      // Check claimable reward - should be base reward * 1 successful vote
      const claimableReward = await GovernorsRewardPay.read.getClaimableReward([voter1.account.address]);
      expect(claimableReward).to.equal(parseEther("0.1")); // 0.1 token
    });

    it("Should apply streak multiplier correctly", async function () {
      const { DAOVoting, GovernorsRewardPay, voter1 } = await loadFixture(deployRewardSystemFixture);
      
      // Set up voter1 to have 1 successful vote and a streak of 3
      ...
      expect(stats.successfulVotes).to.equal(3n);
  ...
  describe("Reward Claiming", function () {
    it("Should allow claiming rewards", async function () {
      const { DAOVoting, GovernorsRewardPay, GovernanceToken, voter1 } = await loadFixture(deployRewardSystemFixture);
      ...
      const initialBalance = await GovernanceToken.read.balanceOf([voter1.account.address]);
      
      // Claim reward
      await GovernorsRewardPay.connect(voter1).write.claimReward();
      
      // Check new token balance
      const newBalance = await GovernanceToken.read.balanceOf([voter1.account.address]);
    ...
    it("Should not allow claiming rewards twice for the same votes", async function () {
      ...
      await expect(
        GovernorsRewardPay.connect(voter1).write.claimReward()
      ).to.be.revertedWith("No new successful votes to claim rewards for");
    });
  });
  ...
  describe("Admin Functions", function () {
    ...
    it("Should allow owner to withdraw funds", async function () {
      const { GovernorsRewardPay, GovernanceToken, deployer } = await loadFixture(deployRewardSystemFixture);
      ...
      const initialDeployerBalance = await GovernanceToken.read.balanceOf([deployer.account.address]);
      
      // Withdraw funds
      await GovernorsRewardPay.write.withdrawFunds([deployer.account.address, withdrawAmount]);
      
      // Check balances
      const newTreasuryBalance = await GovernanceToken.read.balanceOf([GovernorsRewardPay.address]);
      const newDeployerBalance = await GovernanceToken.read.balanceOf([deployer.account.address]);

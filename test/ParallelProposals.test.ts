...
describe("Parallel Proposals Test", function () {
  ...
  async function deployDAOFixture() {
    ...
    for (let i = 0; i < voters.length && i < 10; i++) {
      ...
      await MyGovernor.write.setVotingWeight([voters[i].account.address, 1]);
    }
  ...
  it("Should handle 10 proposals in parallel", async function () {
    const { MyGovernor, voters } = await loadFixture(deployDAOFixture);
    ...
    for (let i = 0; i < NUM_PROPOSALS; i++) {
      ...
      for (let j = 0; j < voters.length && j < 10; j++) {
        ...
        await MyGovernor.write.setCommitted([voter.account.address, true]);
        
        // Create commit hash
        const commitHash = createCommitHash(support, SALT);
        
        // Store commitment
        await MyGovernor.write.storeCommitment([proposalId, voter.account.address, commitHash]);
        
        // Count vote directly
        await MyGovernor.write.countVoteForTest([proposalId, voter.account.address, support, 1n]);
        
        // Record that the vote was revealed
        await MyGovernor.write.setHasRevealed([proposalId, voter.account.address, true]);
        await MyGovernor.write.setRevealedVoteSupport([proposalId, voter.account.address, support]);
        await MyGovernor.write.addVoterToList([proposalId, voter.account.address]);
      }
    }
    ...
    for (let i = 0; i < NUM_PROPOSALS; i++) {
      ...
      await MyGovernor.write.updateVoterStatsForTest([proposalId, proposalSucceeds]);
    }
    ...
    for (let j = 0; j < voters.length && j < 10; j++) {
      ...
      // Get individual stats components
      const successfulVotes = await MyGovernor.read.governorSuccesfulvotes([voter.account.address]);
      const failedVotes = await MyGovernor.read.governorfailedvotes([voter.account.address]);
      const streak = await MyGovernor.read.governorStreak([voter.account.address]);
      
      console.log(`Voter ${j}: successfulVotes=${successfulVotes}, failedVotes=${failedVotes}, streak=${streak}`);
      
      // Each voter should have participated in all proposals
      expect(successfulVotes + failedVotes).to.equal(BigInt(NUM_PROPOSALS));
      
      // Voters who vote FOR should win on even proposals (which succeed)
      // Voters who vote AGAINST should win on odd proposals (which fail)
      const votesFor = j % 2 === 0;
      const expectedSuccesses = votesFor ? 
        Math.ceil(NUM_PROPOSALS / 2) : // FOR voters win on even proposals (half of them)
        Math.floor(NUM_PROPOSALS / 2); // AGAINST voters win on odd proposals (half of them)
      
      expect(successfulVotes).to.equal(BigInt(expectedSuccesses));
    }
  });
}); 